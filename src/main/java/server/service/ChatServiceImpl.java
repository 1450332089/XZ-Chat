...
public class ChatServiceImpl implements ChatService{
    ...
    private Map<Integer, LoggedUser> map = new ConcurrentHashMap<>();    //存放用户信息的集合
    ...
    //增加了新用户，则通知客户机更新用户列表
    @Override
    public void addUser(int id, String name, String ip, int port) {
        ...
        System.out.println("用户"+id+"已存入map中: "+map);
    ...
    //向指定客户机发消息
    @Override
    public void individualCast(int toId, String msg) {
        Iterator<Map.Entry<Integer, LoggedUser>> entries = map.entrySet().iterator();
        while (entries.hasNext()) {
            Map.Entry<Integer, LoggedUser> entry = entries.next();
            LoggedUser user = entry.getValue();
